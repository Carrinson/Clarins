...
import Products from "../body.json";
import CardPopup from "../components/CardPopup";

function Body() {
  const [selectedCard, setSelectedCard] = useState(null);
  const [showMore, setShowMore] = useState(false);
  const initialItems = Products.body ? Products.body.slice(0, 8) : [];
  const [displayedItems, setDisplayedItems] = useState(initialItems);

  const handleShowMore = () => {
    setShowMore(true);
    setDisplayedItems(Products.body);
  };
  ...
  return (
    <div className="gen-container">
      ...
      {!showMore && Products.body && Products.body.length > 8 && (
